#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""mf18_balls.py -- pure-numpy ANALYTIC boundary-ball sampler for the mf17 plain-cup demo.
...
inset delta = 0.5*ps) for:
  ...
  cup_big   (static, world frame; inner radius 0.035, inner height 0.100, wall 0.004)
            Standing directly on the table (lift 0): outer bottom at local z = 0, cavity
            floor at z = 0.004, rim at local z = 0.104.  Real-scale MF-18 mug.
            -> videos/mf18_balls_cup_big.npz
  cup_small (kinematic, cup-local frame; inner radius 0.028, inner height 0.100, wall 0.004)
            Local origin at the OUTER bottom centre, NO lift baked (lift is applied later
            via the kinematic pose).  Rim at local z = 0.104.  Real-scale MF-18 jug.
            -> videos/mf18_balls_cup_small.npz
...
self-check report printed and saved to videos/mf18_balls_report.json:
...
Run:
  PY=/c/Users/admin/.conda/envs/ipbf/python.exe; PYTHONIOENCODING=utf-8 "$PY" mf18_balls.py
"""
...
# ---------------------------------------------------------------- sampling spec
PS = 0.002           # ball spacing == fluid particle spacing [m] (real scale)
...
BODIES = {
    "cup_big": dict(
        r_in=0.035, h_in=0.100, t_wall=0.004, t_bottom=0.004, n_seg=N_SEG,
        lift=0.0,                       # real scale: the mug stands directly on the table
        kinematic=False,
        ...
        obj=os.path.join(ASSETS, "mf18_cup_big.obj"),
    ),
    "cup_small": dict(
        r_in=0.028, h_in=0.100, t_wall=0.004, t_bottom=0.004, n_seg=N_SEG,
        ...
        obj=os.path.join(ASSETS, "mf18_cup_small.obj"),
    ),
}
...
class Body:
    ...
    def finalize(self, dedup_frac=DEDUP_FRAC):
        ...
        # so each wall copy carries half weight -- exactly the mf16-mug convention.  A thin
        # wall (outer face skipped at generation; see build_body) leaves a single shared
        # barrel, which then carries full weight.
        mass_weight = np.ones(len(pts64), dtype=np.float32)
        if bool(np.isin(face_label, "outer_wall").any()):
...
def wall_radial_report(name: str, res: dict, cfg: dict):
    ...
    for (f, L), r_spec in spec_r.items():
        m = (face == f) & (layer == L)
        # thin-wall bodies skip the outer face entirely (see build_body)
        if m.sum() == 0 and f == "outer_wall":
            continue
...
def bottom_seal_report(name: str, res: dict, cfg: dict):
    ...
    # bottom exactly (z = 0 un-lifted).  A thin-wall body skips the outer face; its global
    # minimum is then the deeper floor layer (the cup rests on the table plane, which seals
    # the under-bottom corridor the outer ring existed for).
    z_min = float(pos[:, 2].min())
    z_min_spec = z_ob
    if not bool((face == "outer_wall").any()):
        z_min_spec = z_ob + cfg["t_bottom"] - INSET - PS
...
def build_body(name, cfg, ps, inset, layers, edge_ring=True):
    ...
    # Thin-wall regime (t_wall <= 2.2*ps): every outer-wall layer radius coincides with an
    # inner-wall layer radius (R_OUT-delta == r_in+1.5*ps), so dedup would absorb the whole
    # outer face and the ring-coverage self-check fails on the emptied rings.  The inner
    # wall's two layers lie within ps of BOTH visible surfaces, so they cover the full wall;
    # finalize then gives the single shared barrel full weight.
    if cfg["t_wall"] <= 2.2 * ps:
        nz_out, z_out_bot = 0, 0.0
    else:
...
def main():
    ...
    print(f"== mf18_balls : analytic boundary-ball sampler, PLAIN straight cups ==")
    ...
    for name, cfg in BODIES.items():
        ...
        report["bodies"][name] = dict(
            ...
            npz_path=os.path.join(args.out_dir, f"mf18_balls_{name}.npz"),
        )
    ...
    for name, res in results.items():
        ...
        meta = dict(
            ...
            geometry=dict(
                ...
                source="mf18_balls.py + gen_mug.build_shell (same-source as mf16 mug shell)",
            ),
        ...
        path = os.path.join(args.out_dir, f"mf18_balls_{name}.npz")
    ...
    if not args.no_png:
        ...
        png = top_view_png(
            ...
            os.path.join(args.out_dir, "mf18_balls_top.png"))
        print(f"\n  png: {png}")

    report["pass"] = True
    report_path = os.path.join(args.out_dir, "mf18_balls_report.json")
