...
PRESSURE_SOLVERS = ("DFSPH", "WCSPH")


def build_scene(pressure_solver="DFSPH", scale=None, show_viewer=False, dt=None):
    ...
    if pressure_solver not in PRESSURE_SOLVERS:
        raise ValueError(f"Unknown Smoothed Particle Hydrodynamics (SPH) pressure solver: {pressure_solver}")
    if scale is None:
...
def main():
    parser = argparse.ArgumentParser(description="SPH liquid in the moving teapot reference case")
    parser.add_argument(
        "--pressure-solver",
        type=str.upper,
        choices=PRESSURE_SOLVERS,
        default="DFSPH",
