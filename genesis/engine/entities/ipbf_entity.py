...
@qd.data_oriented
class IPBFEntity(ParticleEntity):
    """
    IPBF-based particle entity.
    ...
    material : Material
        Material properties (e.g., rest density).
    morph : Morph
    ...
    def _add_particles_to_solver(self):
        ...
        # (0 = water, 1 = coffee); material without c_init keeps the previous all-zero default.
        c_init = getattr(self._material, "c_init", 0.0)
        # clamp ownership (multiflow MF-11): 0 = primary boundary, 1 = pitcher container
        boundary_group = getattr(self._material, "boundary_group", 0)
    ...
    def get_state(self):
        """
        Get the current state of the IPBFEntity including positions and velocities.
    ...
    def _get_morph_identifier(self) -> str:
        return f"ipbf_{super()._get_morph_identifier()}"
