...
边界（solid）粒子同样携带密度约束 $C_i^\rho$，其密度由邻域内流体与边界粒子的质量共同累加，计算方式与流体粒子一致。边界粒子位置固定，不参与位置求解与速度更新，其约束仅作为单向源进入邻近流体粒子的局部能量 $E_i$，使靠近壁面的流体粒子在 $\boldsymbol H_i$ 中获得与内部粒子一致的约束曲率。
